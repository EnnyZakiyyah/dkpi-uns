                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $date = empty(strtotime($internasional->jangka_waktu_awal)) ? $internasional->jangka_waktu_awal : Carbon\Carbon::parse(date('Y-m-d', strtotime($internasional->jangka_waktu_awal)))->isoFormat('D MMMM Y'); }}
                                </td>
                                <td>{{ $date = empty(strtotime($internasional->jangka_waktu_akhir)) ? $internasional->jangka_waktu_akhir : Carbon\Carbon::parse(date('Y-m-d', strtotime($internasional->jangka_waktu_akhir)))->isoFormat('D MMMM Y'); }}
                                </td>
                                ...
                                <td style="color: rgb(255, 217, 0)">{{ $internasional->status }}</td>
                                @else
                                <td style="color: red">{{ $internasional->status }}</td>
                                @endif
                                <td>
                                    <a href="/home/mitra/{{ $internasional->id }}" class="badge bg-info"><span
                                            data-feather="eye">lihat</span></a>
                                    @if (Auth::check())
                                    <a href="/mitra/{{ $internasional->id }}/edit" class="badge bg-warning"><span
                                            data-feather="eye">edit</span></a>
                                    <form action="/mitra/{{ $internasional->id }}/delete" method="POST"
                                        class="d-inline">
                                        @method('delete')
                                        @csrf
